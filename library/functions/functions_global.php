...
function printOutput($Output = "", $Page_Title = "")
{
	...
	if(is_null($TSUE["TSUE_Template"]->LoadTemplate("tinymce_init"))){
    ...
    $PAGEID = PAGEID;
    $PAGEFILE = PAGEFILE;
	if(is_null($TSUE["TSUE_Template"]->LoadTemplate("main_javascript"))){
		eval("\$main_javascript = \"" . $TSUE["TSUE_Template"]->LoadTemplate("main_javascript") . "\";");
	}
	else{
		eval("\$main_javascript = \"" . "" . "\";");
	}
    //eval("\$main_javascript = \"" . $TSUE["TSUE_Template"]->LoadTemplate("main_javascript") . "\";");
    $TIMENOW = TIMENOW;
    $VERSION = V;
    $loadjavascriptsCache = $TSUE["TSUE_Template"]->prepareJS();
	if(is_null($TSUE["TSUE_Template"]->LoadTemplate("main"))){
...
function log_strike($membername,$receiver_memberid = 0,  $password = "")
{
    global $TSUE;
...
function sent_mail($to, $subject, $message,  $toname = "",$from = "", $fromname = "", $nl2br = 1, $useTemplate = 1)
{
    global $TSUE;
...
function get_phrase()
{
    global $TSUE;
    $args = func_get_args();
	if(!isset($TSUE["TSUE_Language"]->phrase[$args[0]])){
		$args[0] = "";
	}
	else{
		if(is_null($TSUE["TSUE_Language"]->phrase[$args[0]])){
			$args[0] = "";
		}
		else{
